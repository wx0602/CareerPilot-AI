...
- 每个响应都返回 `X-Request-ID`；客户端也可传入 1～64 位字母、数字、点、下划线或连字符作为请求 ID，以便关联后端日志。
...
常用状态码：`401` 未登录或令牌过期、`403` 越权、`404` 不存在、`409` 流程冲突、`413` 文件过大、`415` 文件无效、`422` 参数错误、`502/503` 外部能力返回无效、尚未接入或数据库暂时不可用。

### `GET /health`

无需认证。除服务状态和 Provider 模式外，返回数据库方言、Alembic 迁移版本；SQLite 环境还返回外键开关和 journal mode。数据库连接失败时返回 `503 database_unavailable`，不暴露连接串或底层异常。
...
### `POST /api/job-recommendations/profile`

请求字段：

```json
{
  "source": "resume | recent_report | combined",
  "session_id": "上传简历所属会话，可选"
}
```

返回可编辑的 `JobCandidateProfile`，包含目标岗位、城市、期望薪资、学历、经验、核心技能、项目经历、薄弱技能和最近报告分数。

### `POST /api/job-recommendations/search`

请求字段：`{"profile": JobCandidateProfile}`。后端优先调用实时岗位源，与 `backend/data/job_seeds.json` 中的企业官方岗位合并去重，然后按“严格 → 放宽薪资和经验 → 放宽城市 → 扩展岗位同义词”筛选。仍不足 5 条或实时源失败时，按岗位方向和技能从种子库返回 Top 5。

响应包含顶层及岗位级 `match_mode`、`fallback_reason`，岗位级 `fallback`、`matched_skills`、`missing_skills` 和 `link_status`。明确失效的链接会降低排序优先级；薪资缺失统一返回“薪资未公开”，不会通过模型估算。

### `POST /api/job-recommendations/interview`

请求字段：`{"profile": JobCandidateProfile, "job": JobRecommendationItem}`。后端创建 `mode="job"` 的训练会话，将岗位 JD、技能要求、原始来源和画像写入面试上下文，返回 `TrainingSessionResponse`。
